...
class IDLMBlockN(DllmAlgorithm):
    ...
    def __init__(self, config: DllmConfig):
        ...
        # Confidence-based accept (no p/q verify): accept spec_i iff the prob
        # the *current* forward assigns to spec_i is >= threshold. First low-conf
        # position truncates and forces a cold-start restart. 0.0 = disabled
        # (falls back to use_spec_verify=False's accept-all behavior).
        # Only active when use_spec_verify=False.
        self.confidence_accept_threshold: float = config.algorithm_config.get(
            "confidence_accept_threshold", 0.0
        )
        self.use_confidence_accept: bool = (
            not self.use_spec_verify and self.confidence_accept_threshold > 0
        )
        ...
        # Per-request acceptance stats (logged on cleanup_request)
        self._req_stats: Dict[int, Dict[str, int]] = {}
        # Optional JSONL dump path (one line per finished request)
        # Default to /tmp/idlm_stats.jsonl so clients can rely on the path.
        # Override via algorithm_config["stats_file"] or env IDLM_STATS_FILE.
        # Set to "" or null in YAML to fully disable.
        import os as _os
        if "stats_file" in config.algorithm_config:
            self._stats_file = config.algorithm_config["stats_file"] or None
        else:
            self._stats_file = _os.environ.get(
                "IDLM_STATS_FILE", "/tmp/idlm_stats.jsonl"
            )
        if self._stats_file:
            logger.info(f"[IDLMBlockN] per-request stats → {self._stats_file}")
        # Step-1 instrumentation: track last-clean max-prob (computed from
        # previous forward's saved logits) as a pre-forward difficulty signal.
        # _req_rounds collects per-round (pre_conf, accept_len, case) tuples;
        # dumped to rounds_file at cleanup_request.
        ...
        if "rounds_file" in config.algorithm_config:
            ...
            self._rounds_file = _os.environ.get(
                "IDLM_ROUNDS_FILE", "/tmp/idlm_rounds.jsonl"
            )
        if self._rounds_file:
            logger.info(f"[IDLMBlockN] per-round signal trace → {self._rounds_file}")
        # Step-2 instrumentation: per-position draft confidence at draft time,
        # paired with the next round's accept_len + all_accept (or discarded=True
        # if upstream reject killed them before verify). Stored in-memory per rpx
        # and flushed to draft_conf_file at cleanup_request.
        self._pending_draft_confs: Dict[int, List[float]] = {}
        self._req_draft_confs: Dict[int, list] = {}
        if "draft_conf_file" in config.algorithm_config:
            self._draft_conf_file = config.algorithm_config["draft_conf_file"] or None
        else:
            self._draft_conf_file = _os.environ.get(
                "IDLM_DRAFT_CONF_FILE", "/tmp/idlm_draft_conf.jsonl"
            )
        if self._draft_conf_file:
            logger.info(f"[IDLMBlockN] per-round draft-conf trace → {self._draft_conf_file}")
        self._timing = {
            "phase1_classify": 0.0,
            "phase2_forward": 0.0,
        ...
        logger.info(
            ...
            f"use_conf_accept={self.use_confidence_accept}, "
            f"conditional_lora={self.conditional_lora}"
        )

    def _record_round(self, rpx: int, pre_conf, accept_len, case: str, committed: int):
        """Step-1 instrumentation: append a per-round record for this request.
        pre_conf is the previous round's last-clean max-softmax-prob (the
        candidate pre-forward signal); accept_len is THIS round's observed
        accepted spec count (vns when all accepted, si when rejected, None
        for cold start where the concept doesn't apply).
        """
    ...
    def _record_draft_conf(
        ...
        """Step-2 instrumentation: one record per forward, carrying both the
        incoming draft (whose verify result we just computed) and this forward's
        outgoing draft (which the NEXT forward will verify — or discard if this
        forward is R).

        case in {V, R, C}:
          V: incoming was all-accepted (accept_len=vns, all_accept=True)
          R: incoming was rejected at si (accept_len=si, all_accept=False)
          C: cold start, no incoming (accept_len=None, all_accept=None)

        Offline pairing:
          (conf_{i-1}, accept_i) = (records[i].incoming_draft_confs, records[i].accept_len)
          (conf_i,   accept_{i+1}) = (records[i].outgoing_draft_confs, records[i+1].accept_len)
        records[i+1].case == 'C' means records[i].outgoing was discarded (i was R).
        """
        if self._draft_conf_file is None:
            return
        records = self._req_draft_confs.get(rpx)
        if records is None:
            records = []
            self._req_draft_confs[rpx] = records
        records.append(
            {
                "case": case,
                "incoming_draft_confs": incoming_confs,
                "accept_len": accept_len,
                "all_accept": all_accept,
                "outgoing_draft_confs": outgoing_confs,
                "vns": self.verify_num_specs,
                "num_masks": self.num_masks,
            }
        )

    def _bump_req_stats(self, rpx: int, accepted: int, is_verify: bool, committed: int):
                ...
                with open(self._rounds_file, "a") as _fp:
                    for round_idx, r in enumerate(rounds):
                        line = {
                            ...
                            "verify_num_specs": self.verify_num_specs,
                        }
                        _fp.write(_json.dumps(line) + "\n")
            except Exception as _e:
                logger.warning(f"[IDLMBlockN] rounds_file write failed: {_e}")
        self._pending_draft_confs.pop(req_pool_idx, None)
        draft_records = self._req_draft_confs.pop(req_pool_idx, None)
        if self._draft_conf_file and draft_records:
            try:
                import json as _json
                with open(self._draft_conf_file, "a") as _fp:
                    for round_idx, r in enumerate(draft_records):
                        # Look-ahead: stamp next forward's verify result so each
                        # row carries (incoming, accept_i) AND (outgoing, accept_{i+1})
                        # without needing an offline join.
                        if round_idx + 1 < len(draft_records):
                            nxt = draft_records[round_idx + 1]
                            next_case = nxt["case"]
                            next_accept_len = nxt["accept_len"]
                            next_all_accept = nxt["all_accept"]
                        else:
                            next_case = None
                            next_accept_len = None
                            next_all_accept = None
                        line = {
                            "rid": rid,
                            "rpx": req_pool_idx,
                            "round": round_idx,
                            **r,
                            "next_case": next_case,
                            "next_accept_len": next_accept_len,
                            "next_all_accept": next_all_accept,
                        }
                        _fp.write(_json.dumps(line) + "\n")
            except Exception as _e:
                logger.warning(f"[IDLMBlockN] draft_conf_file write failed: {_e}")
        s = self._req_stats.pop(req_pool_idx, None)
        if s is not None and s["n_forwards"] > 0:
            vr = s["n_verify_rounds"]
            avg_accept = (s["n_accepted_specs"] / vr) if vr > 0 else 0.0
            tok_per_fwd = s["n_committed"] / s["n_forwards"]
            logger.info(
                f"[IDLMBlockN][req] rpx={req_pool_idx} rid={rid} "
                f"avg_accept_len={avg_accept:.2f}/{self.verify_num_specs} "
                f"verify_rounds={vr} committed={s['n_committed']} "
                f"forwards={s['n_forwards']} "
                f"tok/fwd={tok_per_fwd:.2f}"
            )
            if self._stats_file and rid:
                try:
                    line = {
                        "rid": rid,
                        "rpx": req_pool_idx,
                        ...
                        "tok_per_fwd": tok_per_fwd,
                    }
                    import json as _json
                    with open(self._stats_file, "a") as _fp:
                        _fp.write(_json.dumps(line) + "\n")
                except Exception as _e:
                    logger.warning(f"[IDLMBlockN] stats_file write failed: {_e}")
    ...
    def run(
        ...
        if not has_any_decode:
            ...
            for bid in range(batch_size):
                ...
                last_logit = full_logits[last_idx].detach().clone()
                self._prev_last_logits[rpx] = last_logit
                if self._rounds_file is not None:
                    self._prev_last_clean_conf[rpx] = float(
                        F.softmax(last_logit, dim=-1).max().item()
                    )
                offset += n_new
        ...
        old_draft_confs = [None] * batch_size  # incoming draft confs (from prev round's outgoing)
        ...
        for bid in range(batch_size):
            ...
            old_draft_confs[bid] = self._pending_draft_confs.pop(rpx, None)
        ...
        if prefill_bids:
            ...
            if self._rounds_file is not None:
                _prefill_confs = (
                    F.softmax(prefill_logits, dim=-1).max(dim=-1).values.tolist()
                )
            else:
                _prefill_confs = None
            for k, bid in enumerate(prefill_bids):
                ...
                if _prefill_confs is not None:
                    self._prev_last_clean_conf[rpx] = float(_prefill_confs[k])
        ...
        draft_confs_gpu = None  # per-draft softmax prob at chosen token (instrumentation)
        if self._timing_enabled:
            _t_vs_start = time.perf_counter()

        # ── Fused greedy path: single gather + argmax for verify+sample ──
        # Skip when confidence-accept is active (needs softmax, can't shortcut).
        if self.temperature <= 0 and not self.fast_verify and not self.use_confidence_accept:
            ...
            if combined_indices:
                ...
                # Instrumentation: per-draft softmax confidence at chosen token.
                # Greedy path uses raw-logit softmax (T=0 → no temperature scaling).
                if (
                    self._draft_conf_file is not None
                    and sample_logit_indices
                    and num_masks > 0
                ):
                    _sample_logits = combined_logits[verify_count:]
                    _n_groups = len(sample_logit_indices) // gs
                    _draft_pos = []
                    for _g in range(_n_groups):
                        for _m in range(num_masks):
                            _draft_pos.append(_g * gs + 1 + _m)
                    _dp_t = torch.tensor(_draft_pos, dtype=torch.long, device=device)
                    _draft_logits = _sample_logits[_dp_t]
                    _draft_softmax = F.softmax(_draft_logits, dim=-1)
                    _draft_chosen = all_sample_ids_gpu[_dp_t].long()
                    draft_confs_gpu = _draft_softmax.gather(
                        1, _draft_chosen.unsqueeze(1)
                    ).squeeze(1)
            ...
            if (self.use_spec_verify or self.use_confidence_accept) and verify_bids:
                nv = len(verify_bids)
                ...
                if self.use_confidence_accept:
                    # Confidence-based accept: P_current(spec_i | ctx) >= threshold.
                    # No p/q ratio, no comparison vs draft distribution.
                    scaled = (
                        all_clean_logits if self.temperature == 1.0
                        else all_clean_logits / self.temperature
                    )
                    probs = F.softmax(scaled, dim=-1)
                    spec_probs = probs.gather(
                        1, all_spec_vals_t.unsqueeze(1)
                    ).squeeze(1)
                    all_accepted_gpu = spec_probs >= self.confidence_accept_threshold
                    # Correction token for first reject = argmax of current logits.
                    all_corr_tokens_gpu = all_clean_logits.argmax(dim=-1)
                elif self.fast_verify:
            ...
            if sample_logit_indices:
                ...
                if self.temperature > 0:
                    ...
                    if draft_indices:
                        ...
                        # Instrumentation: per-draft softmax confidence at chosen token
                        if self._draft_conf_file is not None:
                            _ds = draft_probs_all
                            _draft_softmax = F.softmax(_ds, dim=-1)
                            _draft_chosen = all_sample_ids_gpu[draft_indices].long()
                            draft_confs_gpu = _draft_softmax.gather(
                                1, _draft_chosen.unsqueeze(1)
                            ).squeeze(1)
                else:
                    ...
                    # Instrumentation: per-draft softmax confidence (T<=0 fallback)
                    if self._draft_conf_file is not None and num_masks > 0:
                        _n_groups = len(sample_logit_indices) // gs
                        _draft_pos = []
                        for _g in range(_n_groups):
                            for _m in range(num_masks):
                                _draft_pos.append(_g * gs + 1 + _m)
                        _dp_t = torch.tensor(_draft_pos, dtype=torch.long, device=device)
                        _draft_logits = all_sample_logits[_dp_t]
                        _draft_softmax = F.softmax(_draft_logits, dim=-1)
                        _draft_chosen = all_sample_ids_gpu[_dp_t].long()
                        draft_confs_gpu = _draft_softmax.gather(
                            1, _draft_chosen.unsqueeze(1)
        ...
        # Separate sync for instrumentation only (no-op when disabled)
        draft_confs_cpu = (
            draft_confs_gpu.tolist() if draft_confs_gpu is not None else None
        )
        ...
        _saved_conf = {}  # bid -> max-softmax-prob at saved-logit position (for analysis)
        if _logit_save_indices:
            _idx_t = torch.tensor(_logit_save_indices, dtype=torch.long, device=device)
            _gathered = full_logits[_idx_t]
            if self._rounds_file is not None:
                _confs = F.softmax(_gathered, dim=-1).max(dim=-1).values.tolist()
                for k, bid in enumerate(_logit_save_bids):
                    _saved_conf[bid] = float(_confs[k])
            if self.temperature <= 0:
                # Greedy shortcut: just compute argmax, skip expensive clone
                _all_argmax = _gathered.argmax(dim=-1).tolist()
                for k, bid in enumerate(_logit_save_bids):
                    _saved_argmax[bid] = _all_argmax[k]
            else:
                _all_saved = _gathered.detach().clone()
                for k, bid in enumerate(_logit_save_bids):
                    _saved_logits[bid] = _all_saved[k]

        # Map sample_bid_roles → its draft slice start in draft_confs_cpu
        _sample_bid_draft_offset = {}
        if draft_confs_cpu is not None:
            _d_off = 0
            for _b in sample_bid_roles:
                _sample_bid_draft_offset[_b] = _d_off
                _d_off += num_masks
        ...
        for bid in range(batch_size):
            ...
            if bid in reject_at:
                ...
                self._bump_req_stats(rpx, accepted=si, is_verify=True, committed=len(output_tokens))
                if self._rounds_file is not None:
                    self._record_round(
                        rpx,
                        ...
                        accept_len=si,
                        case='R',
                        committed=len(output_tokens),
                    )
                    if bid in _saved_conf:
                        self._prev_last_clean_conf[rpx] = _saved_conf[bid]
                # Draft-conf instrumentation: one record per forward.
                # R case: incoming was rejected at si; outgoing will be discarded
                # (next forward is forced cold start), so we do NOT stash it.
                if self._draft_conf_file is not None:
                    _outgoing = None
                    if (
                        draft_confs_cpu is not None
                        and bid in _sample_bid_draft_offset
                    ):
                        _off = _sample_bid_draft_offset[bid]
                        _outgoing = draft_confs_cpu[_off:_off + num_masks]
                    self._record_draft_conf(
                        rpx, case='R',
                        incoming_confs=old_draft_confs[bid],
                        accept_len=si, all_accept=False,
                        outgoing_confs=_outgoing,
                    )
                ...
                self._bump_req_stats(rpx, accepted=vns, is_verify=True, committed=len(output_tokens))
                if self._rounds_file is not None:
                    self._record_round(
                        rpx,
                        pre_conf=self._prev_last_clean_conf.get(rpx),
                    ...
                    if bid in _saved_conf:
                        self._prev_last_clean_conf[rpx] = _saved_conf[bid]
                # Draft-conf instrumentation: V case writes one record carrying
                # both incoming (just verified, all-accept) and outgoing (next
                # forward will verify); outgoing is also stashed.
                if self._draft_conf_file is not None:
                    _outgoing = None
                    if (
                        draft_confs_cpu is not None
                        ...
                        _off = _sample_bid_draft_offset[bid]
                        _outgoing = draft_confs_cpu[_off:_off + num_masks]
                        self._pending_draft_confs[rpx] = _outgoing
                    self._record_draft_conf(
                        rpx, case='V',
                        incoming_confs=old_draft_confs[bid],
                        accept_len=vns, all_accept=True,
                        outgoing_confs=_outgoing,
                    )
                ...
                if bid in _saved_argmax:
                    ...
                    self._prev_last_logits[rpx] = _saved_logits[bid]
                self._bump_req_stats(rpx, accepted=0, is_verify=False, committed=len(output_tokens))
                if self._rounds_file is not None:
                    self._record_round(
                        rpx,
                        pre_conf=self._prev_last_clean_conf.get(rpx),
                    ...
                    if bid in _saved_conf:
                        self._prev_last_clean_conf[rpx] = _saved_conf[bid]
                # Draft-conf instrumentation: cold start has no incoming; write a
                # record (incoming=None, accept_len=None) so each forward → one
                # record. Outgoing is also stashed for next forward's verify.
                if self._draft_conf_file is not None:
                    _outgoing = None
                    if (
                        draft_confs_cpu is not None
                    ...
                    self._record_draft_conf(
                        rpx, case='C',
                        incoming_confs=None,
                        accept_len=None, all_accept=None,
                        outgoing_confs=_outgoing,
                    )
