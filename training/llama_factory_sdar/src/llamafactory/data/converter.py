        ...
        if self.dataset_attr.load_from in ["script", "file"]:
            if isinstance(medias[0], str):
                for i in range(len(medias)):
                    ...
                    if os.path.isfile(media_path) or os.path.isdir(media_path):
                        medias[i] = media_path
                    else:
...
def align_dataset(
    ...
    For pre-tokenized datasets (data_args.pretokenized=True), the dataset is returned
    as-is without conversion, since it already contains input_ids, attention_mask, labels.
    """
    # Skip conversion for pre-tokenized datasets
    if data_args.pretokenized:
        logger.info_rank0("Skipping dataset conversion for pre-tokenized dataset.")
        return dataset
