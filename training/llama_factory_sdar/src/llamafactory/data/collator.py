...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        fake_input_ids = []
        if (
            self.template.mm_plugin.image_token is not None and sum(batch_imglens) == 0 and sum(batch_vidlens) == 0 and getattr(self.model.config, "model_type", None) != "sdar_v"
        ...
        if (
            self.template.mm_plugin.audio_token is not None and sum(batch_audlens) == 0 and getattr(self.model.config, "model_type", None) != "sdar_v"
        ):  # avoid process hanging in zero3/fsdp case
