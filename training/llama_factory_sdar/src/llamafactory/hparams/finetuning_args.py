...
class FinetuningArguments(
    ...
    ce_alpha: float = field(
        ...
        metadata={
            "help": (
                "Weight for the clean-token CE loss. The total loss is: "
                "task_loss + alpha * clean_ce_loss."
            )
        },
    )
    idlm_loss_type: Literal["ar"] = field(
        default="ar",
        metadata={
            "help": (
                "Loss type for introspective training. "
    ...
    loss_auto_balance: bool = field(
        ...
        metadata={
            "help": (
                "Whether to automatically scale clean CE loss to match task loss magnitude. "
                "When True, clean_ce_loss is scaled by task_loss / clean_ce_loss (detached) "
                "so that ce_alpha becomes a true relative weight."
            )
        },
    )
