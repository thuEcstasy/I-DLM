...
class DataArguments:
    ...
    truncate_mode: Literal["drop", "cut"] = field(
        default="cut",
        metadata={"help": "The strategy to truncate samples longer than `cutoff_len`."},
    )
    block_length: Optional[int] = field(
        default=None,
        metadata={"help": "The block length for blockwise attention models."},
    )
    pretokenized: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether the dataset is pre-tokenized. "
                "When True, expects the dataset to have input_ids, attention_mask, and labels columns."
            )
        },
    )
